...
export default function AuditPage() {
    ...
    // Inspector Tab State (Reuso do Cockpit)
    ...
    async function handleSaveEvaluation(evaluation: string) {
        ...
        try {
            ...
            if (data.ok) {
                // Update local state
                const updated = logs.map(l => l.id === selectedLog.id ? { 
                    ...l, 
                    evaluation, 
                    evaluationNote: evalNote,
                        ...
                        <div className="log-list">
                            {logs.map(log => (
                                <div 
                                    ...
                                    onClick={() => {
                                        setSelectedLog(log);
                                        setEvalNote(log.evaluationNote || "");
                                    }}
                                >
                                    <div className="log-item-meta">
                                        <span>{format(new Date(log.createdAt), "HH:mm:ss dd/MM", { locale: ptBR })}</span>
                                        <span className={`badge ${log.evaluation || 'PENDING'}`}>
                                            {log.evaluation || 'PENDING'}
                                        </span>
                                    </div>
                        ...
                        <div className="audit-workspace">
                            <div className="workspace-top">
                                ...
                                <div className="evaluation-panel">
                                    ...
                                    <div className="eval-buttons">
                                        <button 
                                            className={`btn-eval good ${selectedLog.evaluation === 'GOOD' ? 'selected' : ''}`}
                                            onClick={() => handleSaveEvaluation('GOOD')}
                                            disabled={saving}
                                        >Boa</button>
                                        <button 
                                            className={`btn-eval bad ${selectedLog.evaluation === 'BAD' ? 'selected' : ''}`}
                                            onClick={() => handleSaveEvaluation('BAD')}
                                            disabled={saving}
                                        >Ruim</button>
                                        <button 
                                            className={`btn-eval critical ${selectedLog.evaluation === 'CRITICAL' ? 'selected' : ''}`}
                                            onClick={() => handleSaveEvaluation('CRITICAL')}
                                            disabled={saving}
                                        >Crítica</button>
                                    </div>
                                    <textarea 
                                    ...
                                    {selectedLog.evaluatedAt && (
                                        <small className="audit-timestamp">
                                            Revisado por {selectedLog.evaluatedBy} em {format(new Date(selectedLog.evaluatedAt), "dd/MM/yy HH:mm")}
                                        </small>
                                    )}
                                </div>
                            </div>

                            {/* Inspector (Fase 4 style) */}
                            <div className="audit-inspector">
                                ...
                                <div className="inspector-content-audit">
                                    ...
                                    {activeTab === 'prompt' && (
                                        <div className="prompt-view-audit">
                                            <pre>{selectedLog.details?.invocations?.[0]?.request?.messages?.[0]?.content}</pre>
                                        </div>
                                    )}
                                    {activeTab === 'contexto' && (
                                        <div className="context-view-audit">
                                            <pre>{JSON.stringify(selectedLog.details?.input?.clinicContextSnapshot, null, 2)}</pre>
                                        </div>
                                    )}
                                    {activeTab === 'json' && (
